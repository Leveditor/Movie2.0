...
export default function Similar() {
  ...
  useEffect(() => {
    async function loadFilme() {
      const similarTitle = await api.get(
        `/3/tv/${id}/similar?api_key=5419518a2cef35d1e6fa80c720b89ae7&language=en-US`
      );
  ...
  return (
    <>
      <button
        className="ml-5 mt-5 pl-5 pr-5 bg-red-900 text-white"
                    ...
                    <div className="sm:flex sm:items-start">
                      <div className="mt-3 text-center sm:mt-0 sm:text-left">
                        <div className="grid grid-cols-4">
                          {similiar.results
                            ? similiar.results.map((movie) => (
                                <div className="w-36 pl-5 pt-5">
                                  <Link to={`/details-series/${movie.id}`} onClick={() => setOpen(false)}>
                                    <div>
                                      <img
